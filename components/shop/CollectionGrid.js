...
import { collectionStateFromParams } from '@/lib/browse-state';
import { useBrowseReturn, updateBrowseParams } from '@/hooks/useBrowseReturn';

export default function CollectionGrid({ products }) {
  // Keep product links in static HTML while query-specific controls hydrate.
  return <Suspense fallback={<section aria-label="Collection products" className="mt-10"><div className="grid grid-cols-1 gap-x-6 gap-y-12 min-[360px]:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{products.map((product) => <ProductCard key={product.id} product={product} />)}</div></section>}><FilteredCollection products={products} /></Suspense>;
}

function FilteredCollection({ products }) {
  const { isMember } = useSanctuaryAccess();
  const params = useSearchParams();
  const { type, size, color, available, sort } = collectionStateFromParams(params);
  ...
  return <section ref={listRef} onClickCapture={rememberProduct} aria-label="Collection products" className="mt-10">
    <div className="mb-8 flex flex-wrap items-end gap-4 border-y border-zinc-800 py-5">
      {[[types, type, 'type', 'Product type'], [sizes, size, 'size', 'Size'], [colors, color, 'color', 'Color']].map(([values, value, key, label]) => values.length > 1 &&
        <label key={label} className="min-w-32 flex-1 space-y-2 text-xs text-zinc-300">{label}
          <select className={selectClass} value={value} onChange={(e) => updateBrowseParams({ [key]: e.target.value })}>
      ...
      <label className="min-w-40 flex-1 space-y-2 text-xs text-zinc-300">Sort by
        <select className={selectClass} value={sort} onChange={(e) => updateBrowseParams({ sort: e.target.value === 'featured' ? '' : e.target.value })}>
      ...
      <label className="flex min-h-11 items-center gap-2 text-sm text-zinc-300"><input type="checkbox" checked={available} onChange={(e) => updateBrowseParams({ available: e.target.checked })} />Available to order</label>
    </div>
    ...
    </div> : <div className="py-12 text-center"><p>No items match these filters.</p><button className="mt-4 min-h-11 underline" onClick={() => updateBrowseParams({ type: '', size: '', color: '', available: '' })}>Clear filters</button></div>}
  </section>;
}
