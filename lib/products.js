...
import { PRODUCT_RECOMMENDATIONS_QUERY } from './shopify/queries.js';
import { rankRelatedProducts } from './discovery.js';
...
export async function getRelatedProducts(product, limit = 4) {
  ...
  let ids = [];
  if (isShopifyCatalogEnabled() && product.id?.startsWith('gid://shopify/Product/')) {
    try {
      const data = await shopifyFetch({ query: PRODUCT_RECOMMENDATIONS_QUERY,
        variables: { productId: product.id, country: process.env.SHOPIFY_BUYER_COUNTRY || 'US' },
        timeoutMs: 2500, tags: [`shopify-product:${product.id}`] });
      ids = (data.productRecommendations || []).map((item) => item.id);
    } catch {
      // Recommendations are optional; their failure must never block a product page.
      console.warn('Product recommendations unavailable; using published collection matches');
    }
  }
  return rankRelatedProducts(product, products, ids, limit);
}
