...
export const CART_FRAGMENT = `fragment CommerceCart on Cart {
  ...
  lines(first: 250) {
    ...
    nodes {
      ...
      discountAllocations { discountedAmount { amount currencyCode } ... on CartAutomaticDiscountAllocation { title } ... on CartCodeDiscountAllocation { code } }
      merchandise { ... on ProductVariant { id sku title availableForSale image { url } product { handle title featuredImage { url } } } }
    }
  }
}`;
...
export async function reconcileCart({ cartId, items, isMember = false, attributes = [], checkout = false }, fetcher = query) {
  ...
  const actualVariants = new Map(normalized.items.map(item => [item.shopifyVariantId, item]));
  const hasUnavailableItems = lines.some(line => !actualVariants.has(line.merchandiseId) || actualVariants.get(line.merchandiseId).available === false);
  const needsReview = !basketMatches(lines, normalized.items) || hasUnavailableItems;
  if (needsReview) messages.push('Shopify updated your quantities or availability. Please review your cart before checkout.');
  return { cart: normalized, messages: [...new Set(messages)], needsReview, hasUnavailableItems, issues: basketReviewIssues(lines, normalized.items) };
}
