...
export async function getShopifyVariants(shopifyId) {
  ...
  try {
    ...
    // Transform variants into a usable shape. Shopify's availableForSale is
    // authoritative for whether a specific variant may be purchased. When a
    // variant is unavailable, normalize its finite inventory to 0 so the
    // existing cart inventory guard cannot fall back to product-level stock.
    const variants = product.variants.edges.map(({ node }) => ({
      shopifyVariantId: node.id, // Full GID like gid://shopify/ProductVariant/12345
      title: node.title,
      sku: node.sku || null,
      available: node.availableForSale !== false,
      quantityAvailable: node.availableForSale === false ? 0 : node.quantityAvailable,
