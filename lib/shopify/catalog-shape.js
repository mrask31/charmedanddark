import { readBundleOffer } from './bundle-offer.js';

/** Pure Shopify-to-UI mapping, including Shopify-owned automatic offer previews. */
...
export function mapShopifyProduct(product) {
  let variants = connectionNodes(product.variants).map(mapShopifyVariant);
  ...
  const bundleOffer = readBundleOffer(product.bundleOffer, basePrice, currencyCode, variants.length);
  if (bundleOffer) variants = variants.map(variant => ({ ...variant, price: bundleOffer.price, compareAtPrice: variant.price, memberBasePrice: variant.price }));
  const price = bundleOffer?.price ?? basePrice;
  ...
  return {
    ...
    price,
    bundleOffer,
    memberBasePrice: bundleOffer?.regularPrice ?? null,
    priceRange: {
      min: price,
      max: bundleOffer?.price ?? Number(product.priceRange.maxVariantPrice.amount),
      currencyCode,
    },
    compareAtPrice,
