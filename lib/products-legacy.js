...
import { enrichProductsWithPromotions, enrichProductWithPromotion } from './promotions';
...
export async function getProducts() {
  ...
  try {
    ...
    // Batch-fetch variant summaries for ALL products that have product_variants rows.
    // Previously gated on is_variant_parent, but many synced products have variants
    // without that flag set. Use all product IDs to capture every product with variants.
    const allProductIds = transformed.map((p) => p.id);
    ...
    const shopifyProducts = transformed.filter(
      (p) => p.shopifyVariantId && !p.variantSummary
    );
    ...
    return enrichProductsWithPromotions(transformed);
  } catch (err) {
    ...
    return enrichProductsWithPromotions(csvProducts);
  }
}
...
export async function getProductBySlug(slug) {
  try {
    ...
    return enrichProductWithPromotion(product);
  } catch (err) {
    ...
    const fallback = getCSV(slug);
    return fallback ? enrichProductWithPromotion(fallback) : null;
  }
}

// Additional query functions for the shop page. Every public product query must
// pass through the same promotion engine so sale state follows a product anywhere
// it is rendered rather than depending on which page fetched it.
export async function getProductsByCategory(category) {
  ...
  return enrichProductsWithPromotions((data || []).map(transformProduct));
}
...
// Compatibility for editorial sections and application references during staged
// rollout. This preserves their pre-migration hidden-only eligibility and order.
export async function getProductsByReferences(references = []) {
  const { data, error } = await supabase
    .from('products')
    .select('*')
    .or('hidden.is.null,hidden.eq.false');
  if (error) throw new Error('Product references are temporarily unavailable');
  const rows = data || [];
  const matches = references.map((reference) => rows.find((row) =>
    [row.id, row.shopify_id, row.slug, row.handle, row.shopify_handle].includes(reference)))
    .filter(Boolean);
  return enrichProductsWithPromotions([...new Map(matches.map((row) => [row.id, transformProduct(row)])).values()]);
}
