 ...
 *   2. If priorities match: largest discount wins
...
export async function getPromotionForProduct(product, promotions) {
  ...
  const matches = activePromos.filter((promo) => productMatchesPromotion(promo, product));
  if (!matches.length) return null;

  // Sort by precedence: priority DESC → discount DESC → createdAt DESC
  matches.sort((a, b) => {
    // 1. Priority (higher wins)
    if ((a.priority || 0) !== (b.priority || 0)) {
      return (b.priority || 0) - (a.priority || 0);
    }
    // 2. Discount amount (larger wins)
    const discountA = a.percentage || 0;
    const discountB = b.percentage || 0;
    if (discountA !== discountB) {
      return discountB - discountA;
    }
    // 3. Newest wins (most recent startDate as proxy for creation)
