...
export function getMirrorCandidates(products, { shopifyCatalogEnabled = false, limit = 250, mood = '' } = {}) {
  ...
  let candidates = (Array.isArray(products) ? products : []).filter((product) => {
    ...
    collections: Array.isArray(product.collections) ? product.collections.map((c) => c.handle || '').filter(Boolean) : [],
  }));
  const normalized = String(mood).toLowerCase().slice(0, 300);
  const identity = (p) => `${p.title} ${p.handle} ${p.tags.join(' ')} ${p.collections.join(' ')}`.toLowerCase();
  ...
  const types = [
    [/\bhoodies?\b/, /\bhoodie\b/],
    [/\b(?:shirts?|tees?)\b/, /\b(?:shirt|tee)\b/],
    [/\b(?:dress|dresses)\b/, /\bdress\b/],
    [/\bskirts?\b/, /\bskirt\b/],
    [/\b(?:outfit|clothes|clothing|apparel)\b/, /\b(?:shirt|tee|hoodie|dress|apparel|clothing|top|skirt)\b/],
  ...
  const negativePhrases = normalized.match(/\b(?:no|not|without|avoid)\s+(?:a\s+|any\s+)?[a-z]+/g) || [];
  const positiveText = negativePhrases.reduce((text, phrase) => text.replace(phrase, ''), normalized);
  const excludedTypes = types.filter(([request]) => negativePhrases.some((phrase) => request.test(phrase)));
  candidates = candidates.filter((p) => !excludedTypes.some(([, match]) => match.test(`${identity(p)} ${p.category}`)));
  const requestedTypes = types.filter(([request]) => request.test(positiveText));
  if (requestedTypes.length) {
    candidates = candidates.filter((p) => requestedTypes.some(([, match]) => match.test(`${identity(p)} ${p.category}`)));
  }
  const budget = normalized.match(/\b(under|below|less than|up to|at most|budget(?: of)?|max(?:imum)?)\s*\$?\s*(\d+(?:\.\d{1,2})?)/);
  if (budget) {
    const ceiling = Number(budget[2]);
    const strict = ['under', 'below', 'less than'].includes(budget[1]);
    candidates = candidates.filter((p) => p.price != null && Number.isFinite(Number(p.price)) && (strict ? Number(p.price) < ceiling : Number(p.price) <= ceiling));
  }
