...
export const getDiscoveryCollection = cache(async function getDiscoveryCollection(handle) {
  ...
  const seasonalArchive = handle === 'summerween';
  return { ...definition, description: seasonalArchive ? definition.description : collection?.description || definition.description,
    seo: seasonalArchive ? { title: definition.title, description: definition.description } : collection?.seo, products };
});
