...
export function validate(p,product,catalog){
 check(['8650217816098','8650245734434','8650227351586'].includes(p.shopifyId),'target_refused');
 check(String(product.sync_product.external_id)===p.shopifyId&&product.sync_variants.filter(v=>!p.retiredVariantIds.includes(v.id)).length===p.mappings.length,'product_mismatch');
 check(product.sync_variants.filter(v=>p.retiredVariantIds.includes(v.id)).every(v=>!v.synced&&v.is_ignored),'retired_variant_changed');
 check(catalog.product.id===p.catalogId&&catalog.product.model===p.model,'catalog_mismatch');
 check(p.mappings.length===({'8650217816098':6,'8650245734434':12,'8650227351586':11})[p.shopifyId],'mapping_count_mismatch');
...
export async function run({env=process.env,fetchImpl=fetch,delay=ms=>new Promise(r=>setTimeout(r,ms))}={}){
 ...
 const plans=JSON.parse(await readFile(new URL('./printful-batch4-mappings.json',import.meta.url),'utf8'));
 ...
 for(const p of plans){for(const f of p.files){let file=await req('POST','/files',{url:f.url,filename:`CND_${p.key}_${f.placement}_positioned_300dpi.png`,visible:true});check(Number.isSafeInteger(file.id)&&file.id>0,'file_invalid');fileIds.add(file.id);
  for(let i=0;file.status==='waiting'&&i<12;i++){await delay(3000);file=await req('GET',`/files/${file.id}`)}check(file.status==='ok'&&file.width===f.width&&file.height===f.height&&Math.abs(file.dpi-f.dpi)<1,'file_quality_mismatch');uploads.set(f.url,file);
 ...
 for(const p of plans){const pilot=p.mappings.find(m=>m.size==='M');const order=[pilot,...p.mappings.filter(m=>m.id!==pilot.id)];const files=p.files.map(f=>({type:f.type,id:uploads.get(f.url).id}));
  ...
  const final=await req('GET',`/sync/products/@${p.shopifyId}`);validate(p,final,catalogs.get(p.catalogId));check(final.sync_variants.filter(v=>!p.retiredVariantIds.includes(v.id)).every(v=>v.synced&&!v.is_ignored&&checkFiles(v)),'final_verification_failed');
  const result={key:p.key,shopifyId:p.shopifyId,synced:p.mappings.length,retiredIgnoredVariants:final.sync_variants.filter(v=>p.retiredVariantIds.includes(v.id)).map(v=>({id:v.id,synced:v.synced,ignored:v.is_ignored})),variants:final.sync_variants.filter(v=>!p.retiredVariantIds.includes(v.id)).map(v=>({id:v.id,externalId:String(v.external_id),catalogVariantId:v.variant_id,sku:v.sku,retailPrice:v.retail_price,files:v.files.filter(f=>f.type!=='preview').map(f=>({id:f.id,type:f.type,status:f.status,width:f.width,height:f.height,dpi:f.dpi}))}))};products.push(result);console.log('BATCH_PRODUCT_COMPLETE='+JSON.stringify(result));
 }
 return {status:'complete',storeId:STORE,writes,products};
}
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){try{check(process.argv.length===2,'args_refused');console.log('BATCH4_LINK_RESULT='+JSON.stringify(await run()));}catch(e){console.log('BATCH4_LINK_FAILED='+(/^[a-z_0-9]{1,50}$/.test(e.message)?e.message:'internal_error'));process.exitCode=1}}
