...
try{
 ...
 const ids=[586,862,84,3,266,71,952,422,271,593],products=["8648973418530", "8650224435234", "8650220109858", "8650220339234", "8650220535842", "8650242228258", "8650242424866", "8650242719778", "8650242949154", "8650244390946", "8650244653090", "8650244816930", "8650245505058"];
 const allowed=new Set(['/products',...ids.flatMap(id=>[`/products/${id}`,`/mockup-generator/printfiles/${id}`]),...products.map(id=>`/sync/products/@${id}`)]);
 ...
 for(const id of ids){const c=await get(`/products/${id}`),a=await get(`/mockup-generator/printfiles/${id}`);report.catalog.push({id,model:c.product.model,title:c.product.title,files:c.product.files,variants:c.variants,printAreas:a})}
 ...
 const all=await get('/products');report.replacementCatalog=all.filter(p=>/ringer|crop tank|water bottle|muscle|flowy/i.test(p.title+' '+p.model));console.log('REST_AUDIT='+JSON.stringify(report));
}catch{console.log('REST_AUDIT_FAILED');process.exitCode=1}
