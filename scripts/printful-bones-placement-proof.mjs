...
try {
 ...
 const plans=JSON.parse(await readFile(new URL('./printful-next-batch-plans.json',import.meta.url),'utf8')).filter(p=>p.key==='bones-brews');check(plans.length===1,'plan_mismatch');
...
}catch(e){console.log('BATCH_PROOFS_FAILED='+(/^[a-z_0-9]{1,40}$/.test(e.message)?e.message:'internal_error'));process.exitCode=1}
