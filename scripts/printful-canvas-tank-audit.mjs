...
try{
 ...
 const ids=[3,271],products=["8650220339234","8650220535842","8650244653090"];
 ...
 const all=await get('/products');report.replacementCatalog=all.filter(p=>/ringer|crop tank|water bottle|muscle|flowy/i.test(p.title+' '+p.model));console.log('CANVAS_TANK_AUDIT='+JSON.stringify(report));
}catch{console.log('CANVAS_TANK_AUDIT_FAILED');process.exitCode=1}
