#!/usr/bin/env node
/**
 * Check whether the fixed new Signature tee draft has imported into Printful.
 * Exactly four GET requests; no refresh, import, sync, or artwork writes.
...
const ALLOWED_SCOPES = Object.freeze([...REQUIRED_SCOPES, 'stores_list/read', 'file_library/read']);
export const SHOPIFY_PRODUCT_ID = '8641656848418';
export const SHOPIFY_VARIANT_IDS = Object.freeze([
  "47433168257058",
  "47433168289826",
  "47433168322594",
  "47433168355362",
...
const ERROR_CODES = new Set([
  ...
  'product_mismatch', 'variant_mismatch', 'internal_error',
]);
  ...
  return {
    ...
    async get(path) {
      const nextPath = requests === 0 ? '/oauth/scopes' : requests === 1 ? '/stores'
        : requests === 2 && verifiedListStoreId !== undefined ? `/stores/${verifiedListStoreId}`
        : requests === 3 && verifiedListStoreId !== undefined ? `/sync/products/@${SHOPIFY_PRODUCT_ID}` : null;
...
function checkScopes(result) {
  ...
  // The existing read-only credential may include optional store/file read grants.
  // Every write scope and every unrelated scope is refused.
...
function externalId(value) {
  if (typeof value === 'string' && /^[1-9][0-9]{0,14}$/u.test(value)) return value;
  if (storeIdValid(value)) return String(value);
  return null;
}

function productCounts(result, storeId) {
  if (!object(result) || !object(result.sync_product) || !Array.isArray(result.sync_variants)) fail('invalid_response');
  const product = result.sync_product;
  const variants = result.sync_variants;
  if (!storeIdValid(product.id) || externalId(product.external_id) !== SHOPIFY_PRODUCT_ID) fail('product_mismatch');
  if (Object.hasOwn(product, 'store_id') && product.store_id !== storeId) fail('store_mismatch');
  if (variants.length !== SHOPIFY_VARIANT_IDS.length || !variants.every((variant) => object(variant)
      && storeIdValid(variant.id) && typeof variant.synced === 'boolean'
      && SHOPIFY_VARIANT_IDS.includes(externalId(variant.external_id))
      && (!Object.hasOwn(variant, 'sync_product_id') || variant.sync_product_id === product.id)
      && (!Object.hasOwn(variant, 'is_ignored') || typeof variant.is_ignored === 'boolean'))) fail('variant_mismatch');
  if (new Set(variants.map((variant) => variant.id)).size !== variants.length
      || new Set(variants.map((variant) => externalId(variant.external_id))).size !== variants.length) fail('variant_mismatch');
  ...
  if ((Object.hasOwn(product, 'variants') && product.variants !== variants.length)
      || (Object.hasOwn(product, 'synced') && product.synced !== synced)
      || (Object.hasOwn(product, 'is_ignored') && typeof product.is_ignored !== 'boolean')) fail('variant_mismatch');
  return {
    variants: variants.length, synced_variants: synced, unsynced_variants: variants.length - synced,
    all_synced: synced === variants.length, ignored_variants: variants.filter((variant) => variant.is_ignored === true).length,
    product_ignored: product.is_ignored === true,
  };
}

export async function runImportProbe({ env = process.env, fetchImpl = globalThis.fetch,
  timeoutMs = TIMEOUT_MS, maxResponseBytes = MAX_BYTES, args = [] } = {}) {
  const report = {
    status: 'blocked', mode: 'get_only', permissions_verified: false,
    store_verified: false, product_verified: false, expected_variants: 21, variant_counts: null,
    writes_performed: 0, writes_tested: false, checks: [],
  };
  ...
  try {
    ...
    passed();
    check = 'fixed_draft_import';
    let result;
    try { result = await client.get(`/sync/products/@${SHOPIFY_PRODUCT_ID}`); }
    catch (error) {
      if (error instanceof SafeError && error.code === 'http_not_found') {
        report.status = 'not_imported';
        report.checks.push({ check, status: 'not_found' });
        return report;
      }
      throw error;
    }
    report.variant_counts = productCounts(result, storeId);
    report.product_verified = true;
    passed();
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const report = await runImportProbe({ args: process.argv.slice(2) });
  process.stdout.write(`${JSON.stringify(report, null, 2)}\n`);
  process.exitCode = ['imported', 'not_imported'].includes(report.status) ? 0 : 2;
}
