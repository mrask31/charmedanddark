...
export default async function LastChancePage() {
  ...
  const { retired } = summerweenSeason(catalog);
  return <><main className="mx-auto max-w-7xl px-6 pb-24 pt-12 lg:px-12">
    ...
    <section className="mt-8 border border-[#c9a96e]/40 bg-[#17131b] p-6" aria-labelledby="summerween-farewell">
      <h2 id="summerween-farewell" className="font-serif text-3xl">{retired ? 'Summerween has taken its final bow.' : 'Summerween’s Final Haunt'}</h2>
      <p className="mt-3 max-w-2xl text-sm leading-relaxed text-zinc-300">{retired ? 'Gone for the season. Never forgotten. Summerween returns next summer with all-new designs.' : 'Summerween returns next summer with new designs. This season’s items are made to order and will retire when the ordering window closes.'}</p>
      <Link href={retired ? '/deceased' : '/collections/summerween'} className="mt-4 inline-block py-2 text-sm text-[#c9a96e] underline">{retired ? 'Remember Summerween in Deceased' : 'Explore Summerween'}</Link>
    </section>
